...
@testset "Prefixes" begin
    ...
    ss = Prefixes(
        Dict("test-prefix" => Sentencize.default), prefix_file = "test.txt", lang = "fr")
    @test "another-test-prefix" in keys(ss.non_breaking_prefixes) &&
...
@testset "split_sentence" begin
    ...
    @test split_sentence("This is a paragraph. It contains several sentences. \"But why,\" you ask?") ==
          [
    ...
    @test split_sentence("Hello. No. 1. No. 2. Prefix. 1. Prefix. 2. Good bye.") ==
          ["Hello.", "No. 1.", "No. 2.", "Prefix.", "1.", "Prefix.", "2.", "Good bye."]      # Numeric only

    @test split_sentence("Hello. .NATO. Good bye.") == ["Hello. .NATO. Good bye."]      # Acronym

    @test split_sentence("Foo bar. (Baz foo.) Bar baz.") ==
          ["Foo bar.", "(Baz foo.)", "Bar baz."]      # Sentence within bracket

    # Deutsch
    @test split_sentence(
        "Nie hätte das passieren sollen. Dr. Soltan sagte: \"Der Fluxcompensator war doch kalibriert!\".",
        lang = "de") == ["Nie hätte das passieren sollen.",
